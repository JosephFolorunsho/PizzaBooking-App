...
class RecipeView extends parentView {
  ...
  addHandlerUpdateServings(handler) {
    this._parentEl.addEventListener('click', function (e) {
      ...
      if (!btn) return;
      const goToServ = +btn.dataset.servings;
      if (goToServ > 0) handler(goToServ);
    });
  }
        ...
        ${this._data.ingredients
          .map(ing => {
            ...
            <li class="recipe__ingredient">
              ...
              <div class="recipe__quantity">${this._fraction(
                ing.quantity
              )}</div>
  ...
  _fraction(num) {
    return num ? new Fraction.Fraction(num).toString() : '';
  }
}
